...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const { date, reading, note } = await req.json();
  if (!date || reading === undefined || reading === null || reading === "") {
    return NextResponse.json({ message: "Date and odometer reading are required." }, { status: 400 });
  }

  try {
    await connectDB();
    const doc = await MotorcycleReading.findOneAndUpdate(
      { _id: params.id, userId: user.userId },
      { date: new Date(date), reading: Number(reading), note },
      { new: true }
    );
    if (!doc) {
...
export async function DELETE(_req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    await connectDB();
    const doc = await MotorcycleReading.findOneAndDelete({ _id: params.id, userId: user.userId });
    if (!doc) {
      return NextResponse.json({ message: "Reading not found." }, { status: 404 });
    }
